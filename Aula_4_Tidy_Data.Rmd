...
## 
![](/Users/jameshunter/Documents/UNIFESP/MAD-CB/typicalwideformat.png)
...
![](/Users/jameshunter/Documents/UNIFESP/MAD-CB/bad_planilha.png)

## E Essa Planilha
![](/Users/jameshunter/Documents/UNIFESP/MAD-CB/bad_plan2.png)
...
![](/Users/jameshunter/Documents/UNIFESP/MAD-CB/plan_excel.png)

## Dicionário dos Dados
![](/Users/jameshunter/Documents/UNIFESP/MAD-CB/data_dic.png)
...
![Tidy Data](/Users/jameshunter/Documents/UNIFESP/MAD-CB/tidy_data_diagram.png)   
...
##
 ![](/Users/jameshunter/Documents/UNIFESP/MAD-CB/select_box.png)
...
## Exemplo: `factor` -- Meses do Ano
  ...
  - Imagine a variável mês que tem 4 valores: "jan", "mai", "out", "abr"
...
## Retirar a Variável `gestante` com `dplyr::select`
```{r gest, echo = TRUE, mysize=TRUE, size='\\footnotesize'}
demog <- demog %>% select(-gestante)
glimpse(demog)
```

# `demog` agora tidy, agora `testes`
...
## `codepac` -- Número ou Categoria?

  - Mais útil como categoria
  - Mas não faz problemas como Integer
  
## Descrição de 'copias_cv` de `testes`
```{r desctest, echo = TRUE, mysize=TRUE, size='\\tiny'}
Desc(testes$copias_cv, plotit = FALSE)
```

## Gráfico de `copias_cv`
![](/Users/jameshunter/Documents/UNIFESP/MAD-CB/Desc_plot_cv.png)

## Descrição de 'contagem_cd4` de `testes`
```{r desctest2, echo = TRUE, mysize=TRUE, size='\\tiny'}
Desc(testes$contagem_cd4, plotit = FALSE)
```

## Gráfico de CD4
![](/Users/jameshunter/Documents/UNIFESP/MAD-CB/Desc_plot_cd4.png)


## O Que É a Média de CD4+ para Homens vs Mulheres?

  - Precisamos `sexo` de `demog`
  - Precisamos `contagem_cd4` de `testes`
  - Como ligamos este variáveis
    - `codepac` -- existe nos 2 tibbles
  - Função para ligar esses valores é `join`

## Fazer um Novo Conjunto com Os Dados Que Precisamos
```{r join1, echo = TRUE, mysize=TRUE, size='\\scriptsize'}
cvsexo <- inner_join(demog, testes, by = "codepac") %>% select(codepac, sexo, copias_cv)
Desc(copias_cv ~ sexo, data = cvsexo, plotit = FALSE)
```

## Joins em Geral
![](/Users/jameshunter/Documents/UNIFESP/MAD-CB/joins.png)

## Resumo de Funções e Pacotes de Aula 4
...
## Problemas para Próxima Aula (21/2)

  1. Utilizando um método sem `join`, calcule a média e o desvio padrão de CD8 em nossa amostra para homens e mulheres. Utilize outras funções que aprendemos no tidyverse.
  
  2. Uma nova doença bacteriana de repente apareceu em São Paulo. Depois de seis meses, os epidemiologistas determinaram que a taxa de infecção é 0.05% da população. Os infectologistas de UNIFESP desenvolveram um teste preliminar que identificar corretamente 98% dos casos em pessoas com a doença. Também, pessoas saudáveis que fazem o teste tem uma resposta positiva em 2% dos casos. Você faz o teste. É positivo. Qual é o probabilidade que você tem a doença?
  
  3. Problema 2 é uma aplicação de qual teorema em probabilidade?

## Semana Que Vem

  - Terça
    - Média, Mediana, Skewness e Medidas de Variação
    - Gráficos básicos úteis em análise exploratória dos dados
  - Sexta
    - Mais funções para manipulação de dados em dplyr
      - Summarize, etc.
    - Funções gráficos - Gráficos DIY